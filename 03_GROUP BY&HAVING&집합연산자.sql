...
-- * 각 직급별 총 사원수, 보너스를 받는 사원수, 급여 총합, 평균 급여, 최저급여, 최고 급여 조회
--   (직급코드 오름차순 정렬)
SELECT JOB_CODE, COUNT(*) "총 사원수", COUNT(BONUS) "보너스를 받는 사원수", SUM(SALARY) "급여 총합"
            , FLOOR(AVG(SALARY)) "평균 급여", MIN(SALARY) "최저 급여", MAX(SALARY) "최고 급여"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
SELECT DEPT_CODE, ROUND(AVG(SALARY)) "평균 급여"
FROM EMPLOYEE
WHERE SALARY >= 3000000         -- 사원의 급여가 300만원 이상인 데이터만
GROUP BY DEPT_CODE;
...
SELECT JOB_CODE, SUM(SALARY) "총 급여합"
FROM EMPLOYEE
GROUP BY JOB_CODE
...
-- * 부서별 급여 합, 직급별 급여 합, 부서 내 지급 급여 총합, 전체 직원 급여 총합 조회
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY) "급여 총합"
FROM EMPLOYEE
...
/*
    * SELECT문 (-- 실행순서)
    ...
    SELECT * | 컬럼 AS "별칭" | 함수식 | 연산식                                           -- 5
    FROM 테이블 | DUAL                                                                         -- 1
    WHERE 조건식 (연산자들을 활용하여 작성)                                            -- 2
    GROUP BY 그룹화기준이되는 컬럼 | 함수식                                           -- 3
...
/*
    * 집합 연산자 : 여러 개의 쿼리문을 하나의 쿼리문으로 만들어주는 연산자
    ...
    - UNION  : 합집합 ( 두 쿼리문을 수행한 결과값을 더해줌 )  --> OR와 유사
    - INTERSECT : 교집합 ( 두 쿼리문을 수행한 결과값의 중복되는 부분을 추출해줌 )  --> AND와 유사
    - UNION ALL : 합집합 + 교집합 ( 중복되는 부분이 두번 조회될 수 있음 )
    - MINUS : 차집합 ( 선행 결과값에 후행 결과값을 뺀 나머지 )
*/
-- ** UNION **
-- 부서코드가 D5인 사원 또는 급여가 300만원 초과인 사원들의 정보를 조회 (사번, 사원명, 부서코드, 급여)
